...
import {
  usePmMinAcceptanceCriteria,
  insertMinAcceptanceCriteriaData,
  updateMinAcceptanceCriteriaData,
  deleteMinAcceptanceCriteriaData,
} from "../hooks/pm/use-pm-min-acceptance-criteria";
import {
  Dialog,
  DialogContent,
  DialogDescription,
...
import PmMinAcceptCriteriaDialogForm from "./PmMinAcceptCriteriaDialogForm";

interface PmMinAcceptCriteriaTabProps {
  pmWoId: number; // Passed as a prop to this page
}

const PmMinAcceptCriteriaTab: React.FC<PmMinAcceptCriteriaTabProps> = ({ pmWoId }) => {
  const { data: criteriaList, isLoading, refetch } = usePmMinAcceptanceCriteria(pmWoId);
  ...
  const handleDeleteConfirm = async () => {
    if (criteriaToDelete) {
      ...
      try {
        await deleteMinAcceptanceCriteriaData(criteriaToDelete.id);
        toast({
          title: "Success",
  ...
  const handleFormSubmit = async (formData: any) => {
    try {
      if (editingCriteria) {
        await updateMinAcceptanceCriteriaData(editingCriteria.id, formData);
        toast({
          title: "Success",
        ...
        await insertMinAcceptanceCriteriaData({ ...formData, pm_wo_id: pmWoId });
        toast({
          title: "Success",
...
export default PmMinAcceptCriteriaTab;