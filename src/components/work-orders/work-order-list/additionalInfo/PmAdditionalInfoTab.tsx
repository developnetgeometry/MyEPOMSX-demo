...
import {
  usePmAdditionalInfoData,
  insertPmAdditionalInfoData,
  updatePmAdditionalInfoData,
  deletePmAdditionalInfoData,
} from "../hooks/pm/use-pm-additional-info-data";
import PmAdditionalInfoDialogForm from "./PmAdditionalInfoDialogForm";

interface PmAdditionalInfoTabProps {
  pmWoId: number; // Passed as a prop to this page
}

const PmAdditionalInfoTab: React.FC<PmAdditionalInfoTabProps> = ({ pmWoId }) => {
  const { data: additionalInfo, isLoading, refetch } = usePmAdditionalInfoData(pmWoId);
  ...
  const handleDeleteInfo = async (info: any) => {
    try {
      await deletePmAdditionalInfoData(info.id);
      toast({
        title: "Success",
  ...
  const handleFormSubmit = async (formData: { description: string }) => {
    try {
      if (editingInfo) {
        await updatePmAdditionalInfoData(editingInfo.id, formData);
        toast({
          title: "Success",
        ...
        await insertPmAdditionalInfoData({ ...formData, pm_wo_id: pmWoId });
        toast({
          title: "Success",
...
export default PmAdditionalInfoTab;