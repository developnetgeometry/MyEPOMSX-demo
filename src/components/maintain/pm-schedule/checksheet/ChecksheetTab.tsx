...
import {
  deletePmSchedChecksheetData,
  insertPmSchedChecksheetData,
  updatePmSchedChecksheetData,
  usePmSchedChecksheetData,
} from "../hooks/use-pm-sched-checksheet-data";
import {
  Dialog,
  DialogContent,
  DialogDescription,
...
import AttachmentDialogForm from "@/components/work-orders/work-request/attachment/AttachmentDialogForm";

interface ChecksheetTabProps {
  pmScheduleId: number; // Passed as a prop to this page
}

const ChecksheetTab: React.FC<ChecksheetTabProps> = ({ pmScheduleId }) => {
  const { data: checksheets, isLoading, refetch } = usePmSchedChecksheetData(pmScheduleId);
  ...
  const handleDeleteChecksheet = async (checksheet: any) => {
    try {
      await deletePmSchedChecksheetData(checksheet.id);
      toast({
        title: "Success",
  ...
  const handleFormSubmit = async (formData: { file: File; description?: string }) => {
    try {
      if (editingChecksheet) {
        await updatePmSchedChecksheetData(editingChecksheet.id, {
        ...
        await insertPmSchedChecksheetData({
          file: formData.file,
          description: formData.description,
