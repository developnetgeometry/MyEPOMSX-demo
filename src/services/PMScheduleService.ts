...
import { createPMScheduleDTO, PMSchedule } from "@/types/maintain";
...
export const PMScheduleService = {
    ...
    async getPMScheduleById(id: number): Promise<PMSchedule | null> {
        const { data, error } = await supabase
          .from("e_pm_schedule")
          .select(`*,
                package: package_id(package_no),
    ...
    async createPMSchedule(payload: createPMScheduleDTO): Promise<createPMScheduleDTO> {
        const { data, error } = await supabase
          .from("e_pm_schedule")
          .insert(payload)
          .select()
