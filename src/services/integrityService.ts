...
export const integrityService = {
  ...
  async getPipingData(id: number): Promise<any | null> {
    const { data, error } = await supabase
      .from("e_asset")
      .select(
        `
    ...
    `
      ...
      .eq("id", id)
      // .not("ims_general.pipe_schedule_id", "is", null)
      .single();

    if (error) {
      throw new Error(`Error fetching piping data: ${error.message}`);
    }
    ...
    return {
      general: {
        ...
        description: imsGeneral?.description || null,
        normal_wall_thickness: imsGeneral?.normal_wall_thickness || 0,
        nominal_bore_diameter: imsGeneral?.nominal_bore_diameter || 0,
        pressure_rating: imsGeneral?.pressure_rating || 0,
        pipe_class_id: imsGeneral?.pipe_class_id || 0,
        pipe_schedule_id: imsGeneral?.pipe_schedule_id || 0,
        circuit_id: imsGeneral?.circuit_id || 0,
        line_no: imsGeneral?.line_no || null,
        insulation: imsGeneral?.insulation || false,
        line_h2s: imsGeneral?.line_h2s || false,
        internal_lining: imsGeneral?.internal_lining || false,
        pwht: imsGeneral?.pwht || false,
      ...
      protection: {
        ...
        lining_condition: imsProtection?.lining_condition || null,
        lining_monitoring: imsProtection?.lining_monitoring || null,
        isolation_system_id: imsProtection?.isolation_system_id || 0,
        detection_system_id: imsProtection?.detection_system_id || 0,
        mitigation_system_id: imsProtection?.mitigation_system_id || 0,
        online_monitor: imsProtection?.online_monitor || 0,
        post_weld_heat_treatment:
          imsProtection?.post_weld_heat_treatment || false,
        line_description: imsProtection?.line_description || null,
        replacement_line: imsProtection?.replacement_line || null,
  ...
  async updatePipingData(assetId: number, payload: any): Promise<void> {
    // Fetch asset_detail_id for this asset
    const { data: asset, error: assetError } = await supabase
      .from("e_asset")
      .select("asset_detail_id")
    ...
    if (payload.general) {
      const { error } = await supabase
        ...
        .update({
          ...
          description: payload.general.description,
          normal_wall_thickness: payload.general.normal_wall_thickness,
          nominal_bore_diameter: payload.general.nominal_bore_diameter,
          pressure_rating: payload.general.pressure_rating,
          pipe_class_id: payload.general.pipe_class_id,
          pipe_schedule_id: payload.general.pipe_schedule_id,
          circuit_id: payload.general.circuit_id,
          line_no: payload.general.line_no,
          insulation: payload.general.insulation,
          line_h2s: payload.general.line_h2s,
          internal_lining: payload.general.internal_lining,
          pwht: payload.general.pwht,
    ...
    if (payload.protection) {
      const { error } = await supabase
        ...
        .update({
          ...
          lining_condition: payload.protection.lining_condition,
          lining_monitoring: payload.protection.lining_monitoring,
          isolation_system_id: payload.protection.isolation_system_id,
          detection_system_id: payload.protection.detection_system_id,
          mitigation_system_id: payload.protection.mitigation_system_id,
          online_monitor: payload.protection.online_monitor,
          post_weld_heat_treatment: payload.protection.post_weld_heat_treatment,
          line_description: payload.protection.line_description,
          replacement_line: payload.protection.replacement_line,
          minimum_thickness: payload.protection.minimum_thickness,
        })
        .eq("asset_detail_id", assetDetailId);
      if (error)
        throw new Error(`Failed to update protection: ${error.message}`);
