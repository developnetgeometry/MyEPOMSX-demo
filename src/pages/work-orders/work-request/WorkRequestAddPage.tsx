...
import { insertWorkRequestData } from "../hooks/use-work-request-data";
...
const WorkRequestFormPage: React.FC = () => {
    const navigate = useNavigate();
    const location = useLocation();
    const queryClient = useQueryClient();
    const initialAssetId = location.state?.asset_id || null; // Read asset_id from state
    ...
    const [formData, setFormData] = useState({
        cm_status_id: 1,
        description: "",
        work_request_date: new Date().toISOString().split("T")[0],
    ...
    const systems = useMemo(() => {
        ...
        apsf?.forEach((a) => {
            ...
            if (formData.facility_id && sys.facility_id.id !== formData.facility_id) return;
            map.set(sys.id, sys);
        });
        return Array.from(map.values());
    ...
    const assets = useMemo(() => {
        return (
            apsf?.filter((a) => {
                const pkg = a.package_id;
                const sys = pkg.system_id;
                if (formData.facility_id && sys.facility_id.id !== formData.facility_id) return false;
    ...
    }, [apsf, formData.facility_id, formData.system_id, formData.package_id]);
    ...
    // Auto-populate facility, system, and package based on asset_id during initialization
    useEffect(() => {
        if (initialAssetId) {
            handleSelectChange("asset_id", initialAssetId);
        }
    }, [initialAssetId, apsf]); // Trigger when asset_id or apsf data changes
                        ...
                        {isLoading ? (
                            ...
                            <>
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    ...
                                    {/* ASSET SCE CODE */}
                                    <div className="space-y-2">
                                        <Label htmlFor="asset_sce_code">Asset SCE Code</Label>
                                        <Select
                                            value={formData.asset_id?.toString() || ""}
                                            onValueChange={(v) =>
                                                handleSelectChange("asset_id", v ? parseInt(v) : null)
                                        ...
                                        >
                                            <SelectTrigger id="asset_sce_code" className="w-full">
                                                <SelectValue placeholder="Select Asset SCE Code" />
                                            </SelectTrigger>
                                            <SelectContent>
                                                {assets.map((a) => (
                                                    <SelectItem key={a.id} value={a.id.toString()}>
                                                        {a.asset_sce_id?.sce_code ?? "Not Available"} - { a.asset_name}
                                                    </SelectItem>
                                                ))}
                                            </SelectContent>
                                        </Select>
                                ...
                                <div className="flex justify-end space-x-2">
                                    <Button type="button" variant="outline" onClick={() => navigate("/work-orders/work-request")}>
                                        Cancel
                                    </Button>
